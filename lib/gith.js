...
var filterSettings = function (settings, payload) {
    ...
    ['repo', 'branch', 'file', 'tag'].forEach(function (thing) {
        ...
        if (wat && wat !== '*') {
            if (thing === 'branch' && payload.type && payload.type.match(/deployment|deployment_status/)) { //bypass branch check for deployments
                return true;
            }
            ...
            passed = checks.some(function (check) {
                ...
                if (_.isRegExp(wat)) {
                    ...
                    if (_.isString(check)) {
                        ...
                        if (check.name && _.isString(check.name)) {
                            return check.name.match(wat);
                        }
                        return false;
                    }
    ...
    eventaur.on('payload', function (originalPayload) {
        ...
        if (filterSettings(settings, payload)) {
            ...
            if (payload.type === 'status') {
                payload = gith.simplifyStatusPayload(payload);
                gith.emit('status', payload);
            }
            if (payload.type === 'create') {
                gith.emit('create', payload);
            }
            if (payload.type === 'push') {
                gith.emit('push', payload);
            }
            if (payload.type === 'pull_request') {
                gith.emit('pull_request', payload);
            }
            if (payload.type === 'deployment') {
                gith.emit('deployment', originalPayload);
            }
            if (payload.type === 'deployment_status') {
                gith.emit('deployment_status', originalPayload);
            }
        }
    });
};
...
Gith.prototype.simplifyStatusPayload = function (payload) {
    delete payload.files;
    delete payload.pusher;
    delete payload.reset;
    delete payload.tag;
    payload.context = payload.original.context;
    payload.state = payload.original.state;
    payload.description = payload.original.description;
    return payload;
}
Gith.prototype.simplifyPayload = function (payload) {
    ...
    if (!branch && payload.branches) {
        branch = payload.branches[0].name;
    }
    if (!branch && payload.ref && payload.ref_type === 'branch') {
        branch = payload.ref;
    }
    if (!branch && payload.pull_request) {
    ...
    var simpler = {
        ...
        tag: tag,
        branch: branch,
        repo: payload.repository ? ((payload.repository.owner.name || payload.repository.owner.login) + '/' + payload.repository.name) : null,
        sha: payload.sha,
        ...
        owner: (payload.repository && payload.repository.owner) ? payload.repository.owner.login : null
    };
    ...
    this.server = http.createServer(function (req, res) {
        ...
        req.on("end", function () {
            try {
                ...
                console.log('Gith errored while trying to parse request');
                res.end();
            }
            ...
            res.write('Hook received by Gith!');
            res.end();
        });
